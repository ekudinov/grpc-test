...
import (
	...
	"github.com/ekudinov/grpc-test/backend/history/service/handlers"
...
func NewEndpoints(service pb.HistoryServer) svc.Endpoints {
	// Business domain.
	...
	var (
		...
		clearEndpoint    = svc.MakeClearEndpoint(service)
		addeventEndpoint = svc.MakeAddEventEndpoint(service)
	)
...
func Run(cfg Config) {
	...
	go func() {
		...
		s := grpc.NewServer()
		pb.RegisterHistoryServer(s, srv)
